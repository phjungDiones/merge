		...
		private ACK_MODE RcvCheck(ref string sRcvData)
		{
			...
			if (IsReceived("\n>") == true)
			{
				...
				else if (sRcvData.Contains("#ER") == true)
				{
                    ...
                    //에러코드가 같이 넘어오지 않기 떄문에 에러코드 확인용 커맨드를 보내줘야한다.
                    
                    Global.GlobalDefine.Instance.Status_Aligner_PA300C_b = true;
            ...
            if (Global.GlobalDefine.Instance.Status_Aligner_PA300C_b)
            {
                SendData("#ERST");
                Global.GlobalDefine.Instance.Status_Aligner_PA300C_b = true;
            }
